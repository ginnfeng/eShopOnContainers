    ...
    foreach ($chart in $charts) {
        ...
        if ($useCustomRegistry) {
            ...
            if ($chart -ne "eshop-common")  {       # eshop-common is ignored when no secret must be deployed
                helm install "$appName-$chart" $chart --values app.yaml --values inf.yaml --values $ingressValuesFile --set app.name=$appName --set inf.k8s.dns=$dns  --set "ingress.hosts={$dns}" --set image.tag=$imageTag --set image.pullPolicy=Always  
            }
        }
    }
}
