...
namespace Support.Log
{
    ...
    {
        public TraceInfo(object input)
            : this(input, null, TraceInfo<TActionType>.DefaultActionType)
        {
        }
        ...
        {            
            ...
            if (string.IsNullOrEmpty(rltString))
                return string.Format("{0}, {1,-11},{2}, {3,-16},Inp=[{4}]", tmStr, Action, Success, SourceTypeName.Truncate(truncateLenght), inps);
            return string.Format("{0}, {1,-11},{2}, {3,-16},Inp=[{4}],Result={5}", tmStr, Action, Success, SourceTypeName.Truncate(truncateLenght), inps, rltString);
